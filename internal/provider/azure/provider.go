...
func New(apiKey string, baseURL string, deployment string, client *http.Client) *Provider {
	return &Provider{
		apiKey:     apiKey,
...
func (p *Provider) Chat(ctx context.Context, request biz.ChatRequest) (biz.ChatResponse, error) {
	...
	if err != nil {
		return biz.ChatResponse{}, fmt.Errorf("read Azure response: %w", err)
	}
...
func (p *Provider) do(ctx context.Context, request biz.ChatRequest) (*http.Response, error) {
	if p.apiKey == "" {
		return nil, &biz.ProviderError{Status: http.StatusServiceUnavailable, Code: "provider_not_configured", Message: "Azure API key is not configured"}
	}
	if p.deployment == "" {
		return nil, &biz.ProviderError{Status: http.StatusServiceUnavailable, Code: "provider_not_configured", Message: "Azure deployment is not configured"}
	}
	...
	url := fmt.Sprintf("%s/openai/deployments/%s/chat/completions?api-version=2024-02-01", p.baseURL, p.deployment)
	upstreamRequest, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(body))
	if err != nil {
		return nil, fmt.Errorf("build Azure request: %w", err)
	}
	upstreamRequest.Header.Set("api-key", p.apiKey)

<template>
  ...
  >
    <div class="bg-slate-800 rounded-lg shadow-2xl w-full max-w-md overflow-hidden border border-slate-700">
      <!-- Header -->
      ...
      <div class="px-6 py-6 space-y-6">
        ...
        <div>
          ...
          <div class="relative">
            <input
              v-model="apiKeyInput"
              type="password"
              class="w-full px-3 py-2 bg-slate-900 border border-slate-600 rounded-md text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent sm:text-sm"
              placeholder="sk-..."
              autocomplete="off"
            />
            <button
              v-if="apiKeyInput"
              @click="copyApiKey"
              class="absolute right-2 top-2 p-1 text-slate-400 hover:text-white"
              title="复制"
            >
        ...
        <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-700">
          <button
            v-if="hasApiKey"
            @click="clearApiKey"
            class="px-4 py-2 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-900/20 rounded-md transition-colors"
          >
            清除
          </button>
          <button
        ...
        <div v-if="saveStatus" :class="[
          ...
          saveStatus.type === 'success' ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'
        ]">
          {{ saveStatus.message }}
        </div>
      </div>
